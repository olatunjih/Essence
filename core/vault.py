"""AES-256-GCM SecretsVault with PBKDF2 KDF + XOR fallback."""
# ruff: noqa
# fmt: off
...
def _aesgcm_available() -> bool:
    """Live read of _AESGCM through essence's assembled namespace (rather
    than this module's import-time copy) so mock.patch("essence._AESGCM", ...)
    is honoured by callers in this module."""
    import essence as _essence
    return getattr(_essence, "_AESGCM", _AESGCM)


def _vault_weak_allowed() -> bool:
    import essence as _essence
    return getattr(_essence, "_VAULT_ALLOW_WEAK", _VAULT_ALLOW_WEAK)
...
class SecretsVault:
    ...
    def _save(self) -> None:
        ...
        """
        if not _aesgcm_available() and not _vault_weak_allowed():
        ...
        ver  = self._VER_AES if _aesgcm_available() else self._VER_XOR
