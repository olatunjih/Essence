"""Full Essence test suite (all sub-sections in one file for quick CI runs)."""
# ruff: noqa
# fmt: off
...
# TEST SUITE  (pytest · pytest-asyncio · unittest.mock)
#
# NOTE: pytest / pytest-asyncio are TEST-ONLY dependencies. Importing them
# unconditionally at module scope previously meant `essence <anything>`
# crashed with ModuleNotFoundError on any machine that hadn't separately
# `pip install`-ed the test stack — a direct violation of this file's own
# "zero-friction install" design goal (see header). They are now optional:
# real pytest is used whenever it's installed (so `pytest essence -v` keeps
# full fidelity), and a minimal shim covering the small API surface this
# suite actually touches (`pytest.mark.asyncio`, `pytest.raises`) is used
# otherwise so the module still imports and the CLI still runs.
...
try:
    ...
    class _RaisesShim:
        ...
        def __exit__(self, exc_type, exc_val, exc_tb):
            if exc_type is None:
                raise AssertionError(f"DID NOT RAISE {self.exc_types}")
            return issubclass(exc_type, self.exc_types)

    pytest = _types.SimpleNamespace(mark=_MarkShim(), raises=_RaisesShim)
    pytest_asyncio = _types.SimpleNamespace()
# ══════════════════════════════════════════════════════════════════════════════
# Run: pytest essence -v
# Or:  pytest essence -v --asyncio-mode=auto -k 'test_'
#
# Coverage targets:
#   agent loop, all backend adapters, all tools, memory backends,
#   heartbeat scheduler, security sandbox, MemoryMigrator.
...
def test_critic_result_parses_valid_pass():
...
def test_critic_result_parses_valid_fail():
...
def test_critic_result_safe_blocks_on_bad_json():
    cr = CriticResult.from_json('this is not json at all')
    assert cr.passed is False
...
def test_plugin_loader_scan(tmp_path):
    ...
    (plugin_dir / "test_p.py").write_text(
        "import essence_v21 as essence\n"
        "@essence.plugin_tool('test_loaded_plugin')\n"
